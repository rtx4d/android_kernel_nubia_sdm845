...
#define PA224_DRV_NAME "optical_key_right"
#define DRIVER_VERSION "1.3.0"

#define LOG_TAG "OPTICAL_KEY_RIGHT_PA224"
...
static dev_t const optical_key_right_dev_t = MKDEV(MISC_MAJOR, 105);

extern struct class *optical_key_class;

#ifdef SENSORS_CLASS_DEV
static struct sensors_classdev sensor_optical_key_right_cdev = {
	.name = "optical_key_right_cdev",
	.vendor = "txc",
	.version = 1,
	.handle = SENSOR_HANDLE_OPTICAL_KEY_RIGHT,
	.type = SENSOR_TYPE_OPTICAL_KEY_RIGHT,
	.max_range = "5",
	.resolution = "5.0",
	.sensor_power = "3",
...
static int pa224_pinctrl_init(struct pa224_data *data, struct device *dev)
{
	...
	data->pin_default = pinctrl_lookup_state(data->pinctrl, "Optical_key_right_default");
	if (IS_ERR_OR_NULL(data->pin_default)) {
		SENSOR_LOG_ERROR("lookup default state failed\n");
		return PTR_ERR(data->pin_default);
	}

	data->pin_sleep = pinctrl_lookup_state(data->pinctrl, "Optical_key_right_sleep");
...
static struct of_device_id pa224_match_table[] = {
	{ .compatible = "Optical_key_right_pa224",},
	{ },
};
...
static struct i2c_driver optical_key_right_driver = {
	.driver = {
		.name	= PA224_DRV_NAME,
		.owner	= THIS_MODULE,
		...
		const struct i2c_device_id *id)
{
	...
	SENSOR_LOG_ERROR(" yulhan probe start\n");
	...
	data->optical_key_device = device_create(optical_key_class, NULL, optical_key_right_dev_t, 
		&optical_key_right_driver ,"right");
	...
	data->optical_key_cdev = sensor_optical_key_right_cdev;
...
create_optcial_key_device_failed:
	...
	device_destroy(optical_key_class, optical_key_right_dev_t);
...
{
	SENSOR_LOG_ERROR(" yulhan pa224_init RIGHT \n");
	return i2c_add_driver(&optical_key_right_driver);
}

static void __exit pa224_exit(void)
{
	i2c_del_driver(&optical_key_right_driver);
}

MODULE_AUTHOR("Sensor Team, TXC");
MODULE_DESCRIPTION("Optical key right sensor driver.");
